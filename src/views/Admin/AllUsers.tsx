...
export function AllUsers() {
  ...
  const handleInvite = async (e: any) => {
    e.preventDefault();
    try {
      console.log(`[Admin] Registering user: ${inviteEmail} as ${inviteRole}`);
      ...
      toast.success(`${inviteRole.replace('_', ' ')} registered`);
      setInviteEmail('');
      setInviteName('');
      setInvitePassword('');
      fetchUsers();
             ...
             <form onSubmit={handleInvite} className="space-y-3">
                 <div className="space-y-1">
                   ...
                   <div className="relative">
                     <select 
                       className="w-full px-4 py-2.5 rounded-xl border border-[#E5E7EB] bg-white text-sm outline-none focus:border-[#0A0A0A] transition-all appearance-none"
                       value={inviteRole}
                 ...
                 <Input type="password" label="Password" value={invitePassword} onChange={setInvitePassword} required minLength={8} />
                 <Button type="submit" className="w-full">Register {inviteRole.replace('_', ' ').toLowerCase()}</Button>
             </form>
          </Card>
