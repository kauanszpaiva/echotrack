         ...
         <AnimatePresence mode="wait">
            {step === 1 && (
               <motion.div key="s1" initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} exit={{opacity:0, x:-20}} className="space-y-6">
                  ...
                  <div className="space-y-4">
                     ...
                     <div>
                        <label className="text-sm font-bold block mb-2">Confidence (1-10)</label>
                        <input type="range" min="1" max="10" value={formData.confidence} onChange={e => setFormData(f => ({...f, confidence: Number(e.target.value)}))} className="w-full accent-[#FF7A00]" />
                        <div className="text-center font-bold text-[#FF7A00]">{formData.confidence}</div>
                     </div>
                     <div>
                        <label className="text-sm font-bold block mb-2">Attendance this week (%)</label>
                        <input type="range" min="0" max="100" step="5" value={formData.attendance} onChange={e => setFormData(f => ({...f, attendance: Number(e.target.value)}))} className="w-full accent-[#FF7A00]" />
                        <div className="text-center font-bold text-[#FF7A00]">{formData.attendance}%</div>
                     </div>
            ...
            {step === 4 && (
               <motion.div key="s4" initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} exit={{opacity:0, x:-20}} className="space-y-6">
                  ...
                  <div>
                     <label className="text-sm font-bold block mb-2">Highlights from this week</label>
                     <textarea className="w-full h-24 p-3 bg-[#F5F5F5] border border-[#E5E7EB] rounded-xl outline-none" value={formData.highlights} onChange={e => setFormData(f => ({...f, highlights: e.target.value}))} placeholder="Key wins, moments of growth, breakthroughs..." />
                  </div>
                  ...
                  <div>
                     <label className="text-sm font-bold block mb-2">Overall class experience this week</label>
                     <textarea className="w-full h-24 p-3 bg-[#F5F5F5] border border-[#E5E7EB] rounded-xl outline-none" value={formData.classExperience} onChange={e => setFormData(f => ({...f, classExperience: e.target.value}))} placeholder="How did your classes feel overall? Any observations?" />
                  </div>
            ...
            {step === 7 && (
               <motion.div key="s7" initial={{opacity:0, x:20}} animate={{opacity:1, x:0}} exit={{opacity:0, x:-20}} className="space-y-6">
                  ...
                  <div className="flex justify-between pt-4">
                     <Button variant="outline" onClick={handleBack}>Back</Button>
                     <Button onClick={() => saveReport('SUBMITTED')} disabled={loading}>{loading ? 'Submitting...' : 'Submit Report'}</Button>
                  </div>
               </motion.div>
            )}
         </AnimatePresence>
