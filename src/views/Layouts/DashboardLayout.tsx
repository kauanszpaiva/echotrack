  ...
  return (
    <div className="flex bg-[#FAFAFA] min-h-screen">
      ...
      <main className="flex-1 ml-[260px] flex flex-col min-h-screen">
        <header className="h-16 flex items-center justify-between px-8 border-b border-[#E5E7EB] bg-white sticky top-0 z-40">
           <div>
             {/* Show back button if we are not on the root dashboard of the current role */}
             {location.pathname.split('/').filter(Boolean).length > 1 && (
               <button 
                 onClick={() => navigate(-1)} 
                 className="flex items-center gap-1.5 text-sm font-medium text-[#6B7280] hover:text-[#0A0A0A] transition-colors"
               >
